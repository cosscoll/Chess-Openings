...
// --- Transition "portail" plein écran (voir CSS .page-transition) ---
// Fait grandir un cercle depuis (x,y) jusqu'à couvrir tout l'écran,
// appelle `onCovered` pendant que l'écran est masqué (pour échanger le
// contenu sans à-coup visible), puis referme le cercle pour révéler la
// nouvelle page. Se désactive proprement si l'utilisateur préfère moins
// d'animations.
//
// IMPORTANT (bug corrigé) : certains navigateurs mobiles ne déclenchent
// jamais l'événement 'transitionend' pour clip-path (notamment avec de
// grandes valeurs comme "150vmax"), ce qui laissait le cercle bloqué en
// position "couverte" — d'où l'écran qui restait entièrement noir. On ne
// dépend donc plus uniquement de 'transitionend' : un setTimeout de secours,
// avec la même durée que la transition CSS, force toujours la suite si
// l'événement ne s'est pas déclenché.
const pageTransition = document.getElementById('page-transition');
let portalTransitionActive = false;
function portalTransition(x, y, onCovered) {
  if (portalTransitionActive) { onCovered(); return; } // sécurité : jamais deux transitions superposées
  ...
  const duration = 550;
  const fallbackDelay = duration + 200; // marge de sécurité si transitionend ne se déclenche pas
  pageTransition.style.pointerEvents = 'auto';
  pageTransition.style.transition = 'none';
  pageTransition.style.clipPath = `circle(0px at ${x}px ${y}px)`;
  // force le recalcul de style pour que la transition suivante parte bien de 0px
  void pageTransition.getBoundingClientRect();
  pageTransition.style.transition = `clip-path ${duration}ms var(--ease)`;
  requestAnimationFrame(() => {
    pageTransition.style.clipPath = `circle(150vmax at ${x}px ${y}px)`;
  });
  ...
  function doCover() {
    if (covered) return;
    covered = true;
    pageTransition.removeEventListener('transitionend', onCoverEnd);
    onCovered();
    requestAnimationFrame(() => {
      pageTransition.style.clipPath = `circle(0px at ${x}px ${y}px)`;
    });
    ...
    function doReveal() {
      if (revealed) return;
      revealed = true;
      pageTransition.removeEventListener('transitionend', onRevealEnd);
      pageTransition.style.pointerEvents = 'none';
      portalTransitionActive = false;
    }
    function onRevealEnd(e) { if (e.propertyName === 'clip-path') doReveal(); }
    pageTransition.addEventListener('transitionend', onRevealEnd);
    setTimeout(doReveal, fallbackDelay);
  }
  function onCoverEnd(e) { if (e.propertyName === 'clip-path') doCover(); }
  pageTransition.addEventListener('transitionend', onCoverEnd);
  setTimeout(doCover, fallbackDelay);
}
// Point de départ du cercle : position du clic, ou centre de l'élément
// si l'activation vient du clavier (Entrée/Espace), où clientX/Y valent 0.
...
Object.keys(OPENINGS).forEach((slug, i)=>{
  ...
  card.addEventListener('click', (e) => {
    const { x, y } = pointFromEvent(e, card);
    portalTransition(x, y, () => openOpening(slug));
  });
  li.appendChild(card);
  grid.appendChild(li);
});
...
function handleBack(e) {
  ...
  const { x, y } = pointFromEvent(e, btn);
  portalTransition(x, y, closeOpening);
}
document.getElementById('back-link').addEventListener('click', handleBack);
document.getElementById('back-link-2').addEventListener('click', handleBack);
