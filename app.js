...
Object.keys(OPENINGS).forEach((slug, i)=>{
  ...
  card.addEventListener('click', () => ChessTransition.play(() => openOpening(slug)));
  li.appendChild(card);
  grid.appendChild(li);
});
...
function openOpening(slug) {
  ...
  if (!openingEngine) {
    openingEngine = ChessEngine.create({ canvas: openingCanvas, idle: true });
  } else {
    openingEngine.reset();
    // Le moteur est réutilisé d'une ouverture à l'autre : on force un
    // recalcul de la taille/caméra à chaque fois plutôt que de compter
    // uniquement sur la détection automatique du navigateur (peu fiable
    // sur mobile quand le canevas repasse de "caché" à "visible" —
    // c'est ce qui rendait l'échiquier invisible à la deuxième ouverture).
    openingEngine.resize();
  }
...
function closeOpening() {
  ...
  window.scrollTo(0, 0);
  // Même précaution qu'à l'ouverture : on force le recalcul du plateau
  // d'accueil, qui redevient visible après avoir été caché.
  homeEngine.resize();
}

document.getElementById('back-link').addEventListener('click', () => ChessTransition.play(closeOpening));
document.getElementById('back-link-2').addEventListener('click', () => ChessTransition.play(closeOpening));
